...
const BlogData = (props: Props) => {
  ...
  const { isLoading, data, error,refetch } = useGetAllBlogQuery({},{refetchOnMountOrArgChange:true});
  const [deleteBlog, { error: updateError, isSuccess }] = useDeleteBlogMutation()
  ...
  const rows = (data ? data.blog.map((item: any) => ({
    id: item._id,
    name: item.title,
