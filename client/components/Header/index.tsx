...
import newaccount from "@/components/Auth-cc/newaccount"
import verification from "@/components/Auth-cc/verification"
...
type Props = {
  open: boolean;
  setOpen: (open: boolean) => void; activeItem: number;
  route: string;
...
const Header: FC<Props> = ({ setOpen, route, open, setRoute, activeItem, }) => {
                ...
                user.avatar ? (
                  <Link href="/profile">
                    <Image
                      ...
                      className="cursor-pointer rounded-full border border-black	dark:border-white "
                      // style={{ border: activeItem === 5 ? "2px solid #fff" : "none" }}
                  ...
                  <Link href="/profile">
                     <Image
                      src={`https://ui-avatars.com/api/?name=Default+User&background=random&length=1`}
                      alt="User Avatar"
                      width={25}
                      height={25}
                      onClick={() => setOpen(false)}
